...
// Mostly from http://www.cs.rpi.edu/~moorthy/Courses/os98/Pgms/client.c
// With some additional changes for specifying local port
...
static void usage(const char *argv0)
{
	...
	printf("  -h, --handshake_port=<handshake_port>  use port <handshake_port> of device (default 19000)\n");
	printf("  -s, --host_name=<host_name>  connect to host <host_name>\n");
...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "p:d:i:h:s:g:q:l:",
				long_options, NULL);
		if (c == -1)
			break;
